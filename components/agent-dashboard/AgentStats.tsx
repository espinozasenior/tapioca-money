...
export function AgentStats({ address }: AgentStatsProps) {
  ...
  const successRate =
    stats.totalRebalances > 0 ? (stats.successfulRebalances / stats.totalRebalances) * 100 : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <StatCard title="Total Rebalances" value={stats.totalRebalances} icon="🔄" />
        <StatCard
          title="Success Rate"
      ...
      <div className="space-y-3 rounded-lg border p-4">
        <h4 className="text-sm font-medium text-gray-700">Detailed Statistics</h4>
      ...
      {gainsData?.breakdown && gainsData.breakdown.length > 0 && (
        <div className="rounded-lg border p-4">
          <h4 className="mb-3 text-sm font-medium text-gray-700">Recent Rebalances</h4>
            ...
            {gainsData.breakdown.slice(0, 5).map((item: any, idx: number) => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-gray-500">${item.amount.toFixed(2)}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-lg border bg-gradient-to-r from-green-50 to-blue-50 p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="mt-1 text-lg font-semibold text-green-600">● Active & Monitoring</div>
          </div>
          <div className="text-right">
            <div className="text-sm text-gray-600">Next Check</div>
            <div className="mt-1 text-sm font-medium">Within 5 minutes</div>
          </div>
        </div>
      </div>
    </div>
