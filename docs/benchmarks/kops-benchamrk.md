...
- Permissioned Actions: The system uses a session key framework. The Brain is granted temporary, narrowly-scoped permissions to authorize a specific, pre-defined action (e.g., "deposit 1,000 $HYPE into Hyperbeat"). This ensures the Agent can _never_ operate outside its mandated strategy, providing a powerful safeguard against threats.
