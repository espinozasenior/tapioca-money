...
describe("Fuzz: AES-256-GCM encrypt/decrypt", () => {
  ...
  test("encrypt then decrypt returns original plaintext", async () => {
    ...
    fc.assert(
      fc.property(
        fc.string({ minLength: 1, maxLength: 5000 }).filter((s) => !s.startsWith("encrypted:v1:")),
        (plaintext) => {
  ...
  test("each encryption produces unique ciphertext", async () => {
    ...
    fc.assert(
      fc.property(
        fc.string({ minLength: 1, maxLength: 1000 }).filter((s) => !s.startsWith("encrypted:v1:")),
        (plaintext) => {
  ...
  test("tampered ciphertext throws", async () => {
    ...
    fc.assert(
      fc.property(
        fc.string({ minLength: 1, maxLength: 500 }).filter((s) => !s.startsWith("encrypted:v1:")),
