...
- **ZeroDev smart account:** With the Privy-backed `account`, initialize a ZeroDev smart wallet. Follow the ZeroDev Quickstart (with `KernelClient`). Example:
  ...
  ```
  After deployment, the user’s EOA has an associated _smart account kernel_. Test that this flow succeeds. Use ZeroDev’s session keys for transaction automation【58†L129-L137】.
...
- ** `protocolAdapter.ts` (Protocol Adapter):** Acts as an intermediary between the strategy and vault adapters. It fetches Morpho data (e.g. vault balances, APYs) using GraphQL. This module will be used by the strategy to make informed decisions.
